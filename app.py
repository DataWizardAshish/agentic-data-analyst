...
def main():
    ...
    uploaded_file = st.file_uploader(
        "Upload CSV file", type=["csv"], help="Upload a CSV file to analyze its schema"
    )
        ...
        try:
            ...
            if st.session_state.analysis_results is not None:
                ...
                if results["schema_analysis"]:
                    ...
                    col3.metric(
                        "Memory Usage", f"{schema['summary']['memory_usage_mb']:.2f} MB"
                    )
                    ...
                    for col_data in columns_data:
                        with st.expander(
                            f"**{col_data['column_name']}** - {col_data['business_type']} ({col_data['confidence']} confidence)"
                        ):
                            ...
                            with col_left:
                                ...
                                st.write(
                                    f"- **Pandas Type**: `{col_data['pandas_dtype']}`"
                                )
                                st.write(
                                    f"- **Null Count**: {col_data['null_count']} ({col_data['null_percentage']}%)"
                                )
                                st.write(
                                    f"- **Unique Values**: {col_data['unique_count']}"
                                )
                                st.write(
                                    f"- **Sample Values**: {col_data['sample_values']}"
                                )

                            with col_right:
                                st.markdown("**🤖 AI Interpretation**")
                                st.write(
                                    f"- **Business Type**: {col_data['business_type']}"
                                )
                ...
                if results.get("profile_analysis"):
                    ...
                    # Numeric columns
                    if (
                        profile.get("numeric_analysis")
                        and len(profile["numeric_analysis"]) > 0
                    ):
                        st.markdown("#### 🔢 Numeric Columns")
                        for col_data in profile["numeric_analysis"]:
                            with st.expander(
                                f"**{col_data['column_name']}** - {col_data['pattern_detected']}"
                            ):
                                col_left, col_right = st.columns(2)
                                ...
                                with col_left:
                                    ...
                                    st.write(
                                        f"- **25th Percentile**: {col_data['q25']:.2f}"
                                    )
                                    st.write(
                                        f"- **75th Percentile**: {col_data['q75']:.2f}"
                                    )
                                    st.write(
                                        f"- **Skewness**: {col_data['skewness']:.2f}"
                                    )
                                ...
                                with col_right:
                                    ...
                                    st.warning(
                                        f"🔍 **Suggestion**: {col_data['actionable_suggestion']}"
                                    )
                    ...
                    if summary["total_issues"] == 0:
                        ...
                        for issue in quality["issues_found"]:
                            ...
                            icon = (
                                ...
                                else "⚠️" if severity == "warnings" else "ℹ️"
                            )

                            with st.expander(
                                f"{icon} {issue['type'].replace('_', ' ').title()} - {issue['column']} ({issue.get('count', 0)} issues)"
                            ):
                                col_left, col_right = st.columns(2)
                                ...
                                with col_left:
                                    ...
                                    if "percentage" in issue:
                                        st.write(
                                            f"- **Affected**: {issue['percentage']:.2f}% of data"
                                        )
                ...
                if results.get("ml_recommendations"):
                    ...
                    col1, col2, col3 = st.columns(3)
                    col1.metric(
                        "Primary Use Case", ml_use_case.get("detected_use_case", "N/A")
                    )
                    col2.metric(
                        "Target Variable", ml_use_case.get("target_variable", "N/A")
                    )
                    col3.metric(
                        "ML Readiness Score",
                        f"{ml_use_case.get('suitability_score', '0')}/100",
                    )

                    st.info(
                        f"**Reasoning**: {ml_use_case.get('target_reasoning', 'N/A')}"
                    )

                    if ml_use_case.get("alternative_use_case", "N/A") != "N/A":
                        st.warning(
                            f"**Alternative**: {ml_use_case.get('alternative_use_case')}"
                        )
                    ...
                    with st.expander(
                        "📋 Column-by-Column Transformation Plan", expanded=True
                    ):
                        st.markdown(
                            feature_eng.get("feature_plan", "No plan generated")
                        )
                    ...
                    # Categorical columns
                    if (
                        profile.get("categorical_analysis")
                        and len(profile["categorical_analysis"]) > 0
                    ):
                        st.markdown("#### 🏷️ Categorical Columns")
                        for col_data in profile["categorical_analysis"]:
                            with st.expander(
                                f"**{col_data['column_name']}** - {col_data['cardinality']} unique values"
                            ):
                                col_left, col_right = st.columns(2)
                                ...
                                with col_left:
                                    ...
                                    st.write(
                                        f"- **Unique Values**: {col_data['cardinality']}"
                                    )
                                    st.write(
                                        f"- **Most Common**: {col_data['top_value']} ({col_data['top_frequency']}x)"
                                    )
                ...
                if results.get("deployment_strategy"):
                    ...
                    if results.get("business_communication"):
                        ...
                        with st.expander("🗣️ Key Messages by Audience", expanded=False):
                            st.markdown(
                                biz_comm.get("stakeholder_talking_points", "N/A")
                            )
                    ...
                    if (
                        ...
                        st.info(
                            "👆 Review all analysis above, then generate the complete PRD document"
                        )

                        # Generate PRD Button
                        if st.button(
                            "🚀 Generate PRD", type="primary", use_container_width=True
                        ):
                            ...
                            with st.spinner("📝 Generating comprehensive PRD..."):
                                prd_result = supervisor.generate_prd(
                        ...
                        if (
                            "prd_result" in st.session_state
                            and st.session_state.prd_result
                        ):
                            ...
                            if prd_data["status"] == "success":
                                ...
                                st.error(
                                    "❌ PRD generation failed. Please review the error above."
                                )
                # Show any errors
                if results["errors"]:
